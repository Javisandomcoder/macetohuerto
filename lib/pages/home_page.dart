...
class HomePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    void scheduleForPlant(Plant plant, AppSettings settingsSnapshot) {
      Future<void> future;
      if (settingsSnapshot.remindersPaused ||
          plant.reminderPaused ||
          !plant.reminderEnabled) {
        future = notificationService.cancelForPlant(plant);
      } else {
        future = notificationService.scheduleNextForPlant(
          plant: plant,
    ...
    void openPlantFromNotification(String plantId) {
      Future.microtask(() async {
        if (!context.mounted) return;
        try {
          var plantsList = ref.read(plantsProvider).valueOrNull;
          if (plantsList == null) {
            await ref.read(plantsProvider.notifier).load();
            plantsList = ref.read(plantsProvider).valueOrNull;
          }
          if (plantsList == null || plantsList.isEmpty) return;
          Plant? target;
          for (final candidate in plantsList) {
          ...
          await Navigator.of(context)
              .push(fadeScaleRoute(PlantDetailPage(plant: target)));
        } catch (_) {}
      });
    }

    ref.listen(notificationTapProvider, (previous, next) {
      next.whenData(openPlantFromNotification);
    });

    ref.listen(plantsProvider, (prev, next) {
      final settingsSnapshot = ref.read(settingsProvider);
      final previousList = prev?.valueOrNull ?? const <Plant>[];
      final previous = {
        for (final plant in previousList) plant.id: plant,
      };
      next.whenData((list) {
        Future.microtask(() {
          ...
          for (final plant in list) {
            ...
            final older = previous[plant.id];
            final changed = older == null ||
                older.reminderEnabled != plant.reminderEnabled ||
                older.reminderPaused != plant.reminderPaused ||
                older.wateringIntervalDays != plant.wateringIntervalDays ||
                older.wateringTime != plant.wateringTime ||
                older.lastWateredAt != plant.lastWateredAt ||
                older.plantedAt != plant.plantedAt;
            if (!changed) continue;
            scheduleForPlant(plant, settingsSnapshot);
          }
          for (final entry in previous.entries) {
            if (!seenIds.contains(entry.key)) {
              unawaited(notificationService
                  .cancelForPlant(entry.value)
                  .catchError((_) {}));
            }
          }
        });
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            tooltip: settings.remindersPaused
                ? 'Reanudar recordatorios'
                : 'Pausar recordatorios',
            onPressed: () async {
              final toggled = !settings.remindersPaused;
              await ref
                  .read(settingsProvider.notifier)
                  .setRemindersPaused(toggled);
              final updatedSettings = ref.read(settingsProvider);
              final state = ref.read(plantsProvider);
              state.whenData((list) {
                Future.microtask(() async {
                  for (final p in list) {
                    scheduleForPlant(p, updatedSettings);
                  }
                  if (context.mounted) {
            ...
            icon: Icon(settings.remindersPaused
                      ...
                      children: [
                        ...
                        Expanded(
                          child: DropdownButtonFormField<String?>(
                            initialValue: (locationFilter ?? '').isEmpty
                                ? null
                                : locationFilter,
                            isExpanded: true,
                            decoration: InputDecoration(
                            ...
                            items: [
                              DropdownMenuItem<String?>(
                                  value: null, child: Text(l10n.all)),
                              ...locations.map((loc) =>
                                  DropdownMenuItem<String?>(
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: DropdownButtonFormField<String?>(
                            initialValue: (speciesFilter ?? '').isEmpty
                                ? null
                                : speciesFilter,
                            isExpanded: true,
                            decoration: InputDecoration(
                            ...
                            onChanged: (v) {
                              ref.read(speciesFilterProvider.notifier).state =
                                  v;
                            },
                          ),
                        ),
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          await Navigator.of(context)
              .push(fadeScaleRoute(const PlantFormPage()));
